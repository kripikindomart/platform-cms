...
export class CategoriesService {
   ...
   * Create new category
   */
  async create(dto: CreateCategoryDto, userId: number): Promise<CategoryResponseDto> {
    // BaseRepository.create() handles audit fields (created_by, updated_by, timestamps)
    const item = await this.categoriesRepository.create(this.prepareDataForDb(dto), userId);
  ...
  async update(id: number, dto: UpdateCategoryDto, userId: number): Promise<CategoryResponseDto> {
    ...
    const item = await this.categoriesRepository.update(id, this.prepareDataForDb(dto), userId);
